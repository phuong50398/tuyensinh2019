									      					...
									      					<div class="col-md-6 genal-border" style="bborder-top: 1px solid #CCc;border-left: 1px solid #CCc; border-bottom: 1px solid #CCc; padding: 10px; min-height:{count($v.hoso)*40 + 22}px">
																...
																<br>
										      				   ...
										      				   <p  class="text-left ttct_dshssv1">
                                                                    ...
                                                                    <span>
                                                                        <button type="submit" class="btn btn-danger btm-xs" name="huyhs" onclick="return confirm('Bạn có chắc chắn muôn hủy học hồ sơ này không?')" value="{$val.soBD}_{$v.id}" style="padding: 4px 9px;"><i class="fa fa-window-close" aria-hidden="true"></i></button>
                                                                    </span>
                                                                    {/if}
										      				   </p>
									      					</div>
...
<script>
	$('button[name="inhoso"]').on('click', function(){
        setTimeout(function(){
           location.href= window.location.origin+window.location.pathname+"?sbd="+{$goi[0].soBD};
        }, 500);
    });
	var socket = io("https://hdgsnn2.gov.vn/", {
          transports: ["websocket"]
