	...
	class Cthutaichinh extends MY_Controller
	{
	  ...
	  //   	}
	    ...
	    public function in_hoadon($danhmuc, $session, $dstenhocphi){
	    	...
	    	// $tien2khoan = $this->Mthutaichinh->tien2khoan($masv);
...
// pr($tien2khoan);
	   ...
	   public function timkiem(){
	    ...
	    public function getDanhmuc(){
	    	...
	    	foreach ($data['hocphi'] as $value) {
	    		if($value['mahp'] != 'tt1'){
	    			...
	    			if($value['mahp'] =="hp2" || $value['mahp'] =="sk"){
	    				$data['tong2khoan5nam'] =  $data['tong2khoan5nam'] + $value['giatri'];
	    			}
	    		}
	    		if($value['mahp'] != 'tt2'){
	    			$sum1 = $sum1 + $value['giatri'];
	    			if($value['mahp'] =="hp1" || $value['mahp'] =="sk"){
	    				$data['tong2khoan4nam'] =  $data['tong2khoan4nam'] + $value['giatri'];
	    			}
	    		}
	    ...
	    public function dstenhocphi(){
	    	$dsten_hp1 			= array(
	    		'hp1'   => 'Học phí tạm thu kì I: '.date('Y') .'-'.(date('Y')+1) ,
	    		'hp2'   => 'Học phí tạm thu kì I: ' .date('Y') .'-'.(date('Y')+1),
	    		'sk'   => ' Phí khám sức khỏe',
	    		'the'  => ' Thẻ thư viện',
	    		'tt1'  => ' BH thân thể',
	    		'tt2'  => ' BH thân thể',
	    		'yt'   => ' BHYT',
