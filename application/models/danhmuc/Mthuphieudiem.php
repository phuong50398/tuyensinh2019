...
class Mthuphieudiem extends MY_Model
{
	...
	public function danhsach_thuphieudiem($ngaythu, $nguoithu, $thoigian){
		...
		if($thoigian == 'cangay'){
			...
			$this->db->where('thoigian_thuphieudiem >=', $todaystart);
			$this->db->where('thoigian_thuphieudiem <=', $todayend);
		}
		if($nguoithu != "all"){
	...
	public function danhsach_tragiaybao($ngaythu, $nguoithu, $thoigian){
		...
		if($thoigian == 'cangay'){
			$todaystart = date('Y-m-d 00:00:00');  
			$todayend = date('Y-m-d 23:59:59');  
			$this->db->where('thoigian_tragiaybao >=', $todaystart);
			$this->db->where('thoigian_tragiaybao <=', $todayend);
		}
		if($nguoithu != "all"){
	...
	public function dssv_tragiaybao($date, $macb){
		...
		$this->db->order_by('thoigian_tragiaybao','DESC');
		return $this->db->get('tbl_sinhvien')->result_array();
	}
