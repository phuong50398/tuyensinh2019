 ...
 class Mbaocaothongke extends MY_Model
 {
 	...
 	public function baocaothongke($makhoa){
 		$this->db->select('tbl_nganh.iMa_nganh,tbl_nganh.sTen_Nganh,
 			...
 			SUM(tbl_sinhvien.tongtien_dathu) AS tongsoluongtien,tbl_sinhvien.trangthai');
 	...
 	public function baocao($trangthai, $ss){
 		// pr($makhoa);
 		// $this->db->distince();
 		...
 		if($ss['maquyen']!=1){
 			$this->db->where('n.makhoa',$ss['makhoa']);
 		}
 		
 		$this->db->order_by("SUBSTRING(`hoten`, CHAR_LENGTH(SUBSTRING_INDEX(`hoten`,' ',CHAR_LENGTH(`hoten`)- CHAR_LENGTH(REPLACE(`hoten`,' ','')))) + 1)");
 		$this->db->join('tbl_nganh as n', 'tbl_sinhvien.FK_sMaNganh = n.iMa_nganh');
 		$this->db->join('dm_khoa as k', 'k.makhoa = n.makhoa');
 		return $this->db->get('tbl_sinhvien')->result_array();
 		// pr($this->db->last_query());
 		// return 
 	}
 	public function ruthoso($ss){
 		$this->db->select('tbl_sinhvien.*');
 	...
 	public function baocao_taichinh($nganh=null, $hocphi=null, $thoigian=null,$nguoithu=null,$type=null){
 		$this->db->select('hocphi_sinhvien.*, tbl_sinhvien.*, hocphi_sinhvien.nguoithu_hocphi as ngthu');
 		if(!empty($nganh)){
 		...
 		if($type != "tatcathoigian")
 		{
	 		...
	 		if($type == 'chieu')
			{
	 		...
	 		if($type == 'cangay'){
				...
				$this->db->where('thoigian_thuhp >=', $todaystart);
				$this->db->where('thoigian_thuhp <=', $todayend);
	 		}
 		}
 		if(!empty($hocphi)){
